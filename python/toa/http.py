...
def _dump(body: dict) -> str:
    # default=str so a stray Decimal or date degrades to a quoted string rather
    # than raising at response time.
...
def json_response(status_code: int, body: dict) -> dict:
    ...
    return _response(status_code, _dump(body), JSON_CONTENT_TYPE)
...
def json_envelope(status_code: int, body: dict) -> dict:
    """Result envelope for a Lambda invoked directly through the SDK.

    This is the "No-CORS" version of the response. Used for lambdas 
    not fronted by an API Gateway
    """

    return {"statusCode": status_code, "body": _dump(body)}
